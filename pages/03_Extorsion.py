...
def cargar_datos():
    ...
    while True:
        url = (
            f"https://www.datos.gov.co/resource/q2ib-t9am.csv?"
...
st.set_page_config(page_title="Extorsion", layout="wide")
...
st.markdown(
    """
Analisis comparativo presidencial utilizando datos abiertos oficiales
...
st.markdown(
    """
### Fuente poblacion DANE

Proyecciones oficiales de poblacion:
...
if "ext_periodo" not in st.session_state:
    st.session_state["ext_periodo"] = "Año Gobierno"
eje_x = st.selectbox(
    "Selecciona periodo:",
...
presidente_mapa = st.selectbox(
    ...
    key="ext_map_presidente",
)

if "ext_map_periodo" not in st.session_state:
    st.session_state["ext_map_periodo"] = "Año Gobierno"
