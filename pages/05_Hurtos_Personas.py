...
with st.spinner("Estamos cargando los datos oficiales. Espera un momento, ya casi queda listo."):
    df = cargar_datos()
...
if "hp_periodo" not in st.session_state:
    st.session_state["hp_periodo"] = "Año Gobierno"
eje_x = st.selectbox(
    "Selecciona periodo:",
...
with st.spinner("Construyendo tablas y métricas comparativas. No te desanimes, ya vamos."):
...
with st.spinner("Generando las gráficas. Un momento más y ya puedes explorarlas."):
...
if "hp_map_periodo" not in st.session_state:
    st.session_state["hp_map_periodo"] = "Año Gobierno"
periodo_mapa = st.selectbox("Periodo para mapas:", ["Año Gobierno", "Trimestre Gobierno", "Mes Gobierno"], key="hp_map_periodo")
...
with st.spinner("Cargando mapas y tablas geográficas. Gracias por esperar, ya casi terminamos."):
    ...
    fig_mapa_depto.update_layout(height=560, margin={"r": 0, "t": 0, "l": 0, "b": 0})
